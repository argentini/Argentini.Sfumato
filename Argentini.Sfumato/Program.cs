...
internal class Program
{
	private static async Task Main(string[] args)
	{
		...
		await Console.Out.WriteLineAsync(Strings.ThickLine.Repeat(appState.MaxConsoleWidth));
		await Console.Out.WriteLineAsync("Sfumato: The lean, modern, CSS generation tool powered by Sass");
		...
		{
            var yaml = await Storage.ReadAllTextWithRetriesAsync(Path.Combine(appState.YamlPath, "sfumato-complete.yml"), AppState.FileAccessRetryMs, cancellationTokenSource.Token);
		...
		await Console.Out.WriteLineAsync(Strings.ThinLine.Repeat(appState.MaxConsoleWidth));
