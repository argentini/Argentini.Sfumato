...
public class BackdropContrast : ScssUtilityClassGroupBase 
{
    ...
    public override string GetStyles(CssSelector cssSelector)
    {
        ...
        if (ProcessDictionaryOptions(cssSelector.AppState.EffectsFiltersOneBasedPercentageOptions, cssSelector, "backdrop-filter: contrast({value});", AppState, out Result))
            return Result;
        ...
        if (ProcessArbitraryValues("number", cssSelector, "backdrop-filter: contrast({value});", AppState, out Result))
            return Result;
