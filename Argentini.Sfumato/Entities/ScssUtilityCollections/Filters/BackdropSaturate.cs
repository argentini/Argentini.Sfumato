...
public class BackdropSaturate : ScssUtilityClassGroupBase 
{
    ...
    public override string GetStyles(CssSelector cssSelector)
    {
        ...
        if (ProcessDictionaryOptions(cssSelector.AppState.EffectsFiltersOneBasedPercentageOptions, cssSelector, "backdrop-filter: saturate({value});", AppState, out Result))
            return Result;
        ...
        if (ProcessArbitraryValues("number", cssSelector, "backdrop-filter: saturate({value});", AppState, out Result))
            return Result;
